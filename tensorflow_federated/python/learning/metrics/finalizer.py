...
def check_keras_metric_config_constructable(
  ...
  if extra_args:
    # TODO(b/197746608): Remove the suggestion of updating `get_config` if that
    ...
    raise TypeError(f'Metric {metric_type_str} is not constructable from '
                    ...
                    f'{extra_args}. Pass the metric constructor instead, or '
                    'update the `get_config()` in the metric class to include '
