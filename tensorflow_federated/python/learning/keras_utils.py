...
import collections
from typing import List, Optional, OrderedDict, Sequence, Union, Callable
import warnings
...
def from_keras_model(
  ...
  if metric_constructors is None:
    ...
    py_typecheck.check_type(metric_constructors, list)
    for metric_constructor in metric_constructors:
      if not callable(metric_constructor):
        raise TypeError(
            f'Metric constructor {metric_constructor} is not callable.')
  ...
  return _KerasModel(
      ...
      metrics=metrics,
      metric_constructors=metric_constructors)
...
class _KerasModel(model_lib.Model):
  ...
  def __init__(self, keras_model: tf.keras.Model, input_spec,
               ...
               metric_constructors: List[Callable[[],
    ...
    if not metrics and metric_constructors:
      for metric_constructor in metric_constructors:
        metric = metric_constructor()
        if not isinstance(metric, tf.keras.metrics.Metric):
          raise TypeError(
              f'Metric constructor {metric_constructor} is not a no-arg '
              'callable that creates a `tf.keras.metrics.Metric`.')
    ...
    self._loss_metric_constructor = _WeightedMeanLossMetric
    self._loss_metric = self._loss_metric_constructor()
    ...
    def federated_output(local_outputs):
      if self._metric_constructors:
        return federated_aggregate_keras_metric(self.get_metric_constructors(),
                                                local_outputs)
  ...
  def get_metric_constructors(self):
    return self._metric_constructors + [self._loss_metric_constructor]
